...
-----------------------
trainperson1 =
  Person {headache = True, runnyNose = False, cough = False, cold = Just False}

trainperson2 =
  Person {headache = True, runnyNose = False, cough = True, cold = Just True}

trainperson3 =
  Person {headache = True, runnyNose = True, cough = False, cold = Just True}

trainperson4 =
  Person {headache = False, runnyNose = False, cough = False, cold = Just False}

trainperson5 =
  Person {headache = False, runnyNose = True, cough = False, cold = Just True}

trainperson6 =
  Person {headache = False, runnyNose = True, cough = True, cold = Just True}

trainperson7 =
  Person {headache = False, runnyNose = False, cough = True, cold = Just False}

trainperson8 =
  Person {headache = True, runnyNose = True, cough = False, cold = Just True}

trainperson9 =
  Person {headache = True, runnyNose = False, cough = False, cold = Just True}

trainperson10 =
  Person {headache = False, runnyNose = True, cough = False, cold = Just False}
...
testperson :: Person
testperson =
  Person {headache = False, runnyNose = True, cough = True, cold = Nothing}

testperson2 :: Person
testperson2 =
  Person {headache = False, runnyNose = False, cough = False, cold = Nothing}
...
countField' field value personlist =
  length . filter (== value) $ map field personlist

--Probability of any field being True in the whole dataset
----------------------------------------------------------
p_field :: (Person -> Bool) -> [Person] -> Float
p_field field personlist =
  fromIntegral (countField field personlist) / (fromIntegral $length personlist)

--Probability of any field being of a certain value in the whole dataset
----------------------------------------------------------
p_field' :: (Person -> Bool) -> Bool -> [Person] -> Float
p_field' field value personlist =
  fromIntegral (countField' field value personlist) /
  (fromIntegral $length personlist)

--Prior probability of having a cold
--------------------------------------
p_cold :: [Person] -> Float
p_cold personlist =
  fromIntegral (countCold personlist) / (fromIntegral $length personlist)
...
p_field_cold field value personlist =
  fromIntegral
    (countField' field value (filter (\p -> cold p == Just True) personlist)) /
  (fromIntegral $length $filter (\p -> cold p == Just True) personlist)

--Probability of any field being a certain value in persons with a cold
----------------------------------------------------------
p_field_noCold :: (Person -> Bool) -> Bool -> [Person] -> Float
p_field_noCold field value personlist =
  fromIntegral
    (countField' field value (filter (\p -> cold p == Just False) personlist)) /
  (fromIntegral $length $filter (\p -> cold p == Just False) personlist)

--conditional probabilities multiplied
--('naive' assumption of independence of the predictors)
---------------------------------------
p_cond :: Person -> [Person] -> Float
p_cond person traindata =
  p_field_cold headache (headache person) traindata *
  p_field_cold runnyNose (runnyNose person) traindata *
  p_field_cold cough (cough person) traindata

p_cond_noCold :: Person -> [Person] -> Float
p_cond_noCold person traindata =
  p_field_noCold headache (headache person) traindata *
  p_field_noCold runnyNose (runnyNose person) traindata *
  p_field_noCold cough (cough person) traindata

--prior probabilities multiplied
--------------------------------
p_priors :: Person -> [Person] -> Float
p_priors person traindata =
  p_field' headache (headache person) traindata *
  p_field' runnyNose (runnyNose person) traindata *
  p_field' cough (cough person) traindata

--probability of a person having a cold given a certain combination of symptoms
-------------------------------------------------------------------------------
p_cold_person :: Person -> [Person] -> Float
p_cold_person person traindata =
  p_cond person traindata * p_cold traindata / (p_priors person traindata)

--probability of a person not having a cold given a certain combination of symptoms
-------------------------------------------------------------------------------
p_noCold_person :: Person -> [Person] -> Float
p_noCold_person person traindata =
  p_cond_noCold person traindata * p_noCold traindata /
  p_priors person traindata
...
classifyNB person traindata =
  putStrLn $
  "Prediction: " ++
  outcome ++ " P(no cold): " ++ (show p_no) ++ ", P(cold): " ++ (show p_yes)
  where
    outcome
      | p_no >= p_yes = "no cold"
      | otherwise = "cold"
